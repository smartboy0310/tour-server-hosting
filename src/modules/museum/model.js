...
class Museums extends PG {
   ALL_MUSEUMS (page, limit) {
      return this.fetchAll(`
         ...
         FROM
               special_object
         WHERE 
                  object_is_delete = false AND object_add_type = 'museum'
         ORDER BY
                  object_id DESC
         OFFSET $1 LIMIT $2 
      `,(page-1)*limit, limit)
   ...
   MUSEUMS_BY_REGION(reg_id) {
      ...
      FROM
            special_object
      WHERE 
               object_is_delete = false AND object_add_type = 'museum' AND object_status = true AND region_id = $1
      ORDER BY
               object_id DESC
   `, reg_id)
   }

   MUSEUMS_BY_SHRINE(shrine_id) {
      return this.fetchAll(`
      SELECT   
               object_id AS id,
               object_name_oz AS name_oz,
      ...
      FROM
            special_object
      WHERE 
               object_is_delete = false AND object_add_type = 'museum' AND object_status = true AND shrine_id = $1
      ORDER BY
               object_id DESC
   `, shrine_id)
   }

   SINGLE_ACTIVE_MUSEUM(object_id) {
      return this.fetch(`
         SELECT   
                  object_id AS id,
                  object_name_oz AS name_oz,
         ...
         FROM
               special_object 
         WHERE 
                  object_id = $1 AND object_add_type = 'museum' AND  object_is_delete = false AND object_status = true
      `, object_id)
   }

   SEARCH_MUSEUMS (search_data, page, limit) {
      return this.fetchAll(`
         SELECT   
                  object_id AS id,
                  object_name_oz AS name_oz,
         ...
         FROM
               special_object
         WHERE 
                  object_is_delete = false AND object_add_type = 'museum' AND (object_name_oz ILIKE $1 OR object_name_uz ILIKE $1 OR object_name_ru ILIKE $1 OR object_name_en ILIKE $1 OR object_info_oz ILIKE $1 OR object_info_uz ILIKE $1 OR object_info_ru ILIKE $1 OR object_info_en ILIKE $1)
         ORDER BY
                  object_id DESC
        OFFSET $2 LIMIT $3  
   ...
   COUNT_MUSEUMS_SEARCH(search_data) {
      return this.fetch(`
         SELECT 
               COUNT(*) 
         FROM
   ...
   ADD_MUSEUM(name_oz, name_uz, name_ru, name_en, title_oz, title_uz, title_ru, title_en, info_oz, info_uz, info_ru, info_en, address_oz, address_uz, address_ru, address_en, location_x, location_y, phone, work_time, link, photo, photo_name, type_oz, type_uz, type_ru, type_en, status, region_id, shrine_id){
      return this.fetch(`
         INSERT INTO
                     special_object ( 
                           object_name_oz,
   ...
   UPDATE_MUSEUM(id, name_oz, name_uz, name_ru, name_en, title_oz, title_uz, title_ru, title_en, info_oz, info_uz, info_ru, info_en, address_oz, address_uz, address_ru, address_en, location_x, location_y, phone, work_time, link, photo, photo_name, type_oz, type_uz, type_ru, type_en, status, region_id, shrine_id){
      return this.fetch(`
         UPDATE
                  special_object 
         SET
   ...
   EDIT_MUSEUM(object_id, object_status) {
      return this.fetch(`
         UPDATE
                  special_object
         SET
