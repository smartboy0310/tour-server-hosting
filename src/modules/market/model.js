...
class Markets extends PG {
   ALL_MARKETS (page, limit) {
      return this.fetchAll(`
         ...
         FROM
               special_object
         WHERE 
                  object_is_delete = false AND object_add_type = 'market'
         ORDER BY
                  object_id DESC
         OFFSET $1 LIMIT $2 
      `,(page-1)*limit, limit)
   ...
   MARKETS_BY_REGION (reg_id) {
      return this.fetchAll(`
         ...
         FROM
               special_object
         WHERE 
                  object_is_delete = false AND object_add_type = 'market' AND object_status = true AND region_id = $1
         ORDER BY
                  object_id DESC
      `, reg_id)
   }

   MARKETS_BY_SHRINE (shrine_id) {
      return this.fetchAll(`
         SELECT   
                  object_id AS id,
                  object_name_oz AS name_oz,
         ...
         FROM
               special_object
         WHERE 
                  object_is_delete = false AND object_add_type = 'market' AND object_status = true AND shrine_id = $1
         ORDER BY
                  object_id DESC
      `, shrine_id)
   }

   SINGLE_ACTIVE_MARKET (object_id) {
      return this.fetch(`
         SELECT   
                  object_id AS id,
                  object_name_oz AS name_oz,
         ...
         FROM
               special_object
         WHERE 
                  object_id = $1 AND object_is_delete = false AND object_add_type = 'market' AND object_status = true
      `, object_id)
   }

   SEARCH_MARKETS (search_data, page, limit) {
      return this.fetchAll(`
         SELECT   
                  object_id AS id,
                  object_name_oz AS name_oz,
   ...
   COUNT_MARKETS_SEARCH(search_data) {
      return this.fetch(`
         SELECT 
               COUNT(*) 
         FROM
   ...
   SINGLE_MARKET (object_id) {
      return this.fetch(`
         ...
         FROM
               special_object 
         WHERE 
                  object_id = $1 AND object_add_type = 'market'
      `, object_id)
   }

   SELECT_MARKET( object_id ) {
      return this.fetch(`
         SELECT
                  object_photo AS photo,
