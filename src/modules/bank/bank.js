...
module.exports = {
   GET: async (req, res) => {
      try {
         ...
         if (search_data) {
            ...
            const foundData = await model.SEARCH_BANKS(`%${search_data}%`, page, limit)
            ...
            const foundData = await model.ALL_BANKS(page, limit)
   ...
   GET_ACTIVE: async (req, res) => {
      try {
         ...
         if (foundByShrine) {
            ...
            name.oz = foundByRegion?.name_oz
            name.uz = foundByRegion?.name_uz
            name.ru = foundByRegion?.name_ru
            name.en = foundByRegion?.name_en

            title.oz = foundByRegion?.title_oz
            title.uz = foundByRegion?.title_uz
            title.ru = foundByRegion?.title_ru
            title.en = foundByRegion?.title_en

            sendData.id = foundByRegion?.id
            sendData.name = name
            sendData.title = title
            sendData.photo = foundByRegion?.photo
   ...
   POST: async (req, res) => {
      try {
         ...
         const createBank = await model.ADD_BANK(name_oz, name_uz, name_ru, name_en, title_oz, title_uz, title_ru, title_en, info_oz, info_uz, info_ru, info_en, address_oz, address_uz, address_ru, address_en, location_x, location_y, phone, work_time, link, photo, photo_name, type_oz, type_uz, type_ru, type_en, status, region_id, shrine_id)
